...
public class UntagTaskWithTagsCommandHandlerTest {
	
	UIContext context = new UIContextTest(FinApplicationSandbox.INSTANCE);
	UntagTaskWithTagsCommandHandler untag = new UntagTaskWithTagsCommandHandler();
	...
	public void testExecuteCommands() throws FinProductionException {
		...
		int n = commands.size();
		for (int i=0; i<n; i++){			
			res = untag.executeCommands(commands.get(i), arguments.get(i), context);
